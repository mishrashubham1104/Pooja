  ...
  return (
    <div className="bg-cream-50">
      ...
      <section className="py-20 px-4 max-w-7xl mx-auto">
        ...
        <div className="flex md:grid md:grid-cols-2 lg:grid-cols-4 gap-6 overflow-x-auto hide-scrollbar pb-4 md:pb-0">
          {services.map((srv, i) => (
            <motion.div
              whileHover={{ y: -6 }}
              key={i}
              className="relative flex-shrink-0 w-72 md:w-auto bg-white rounded-2xl shadow-md border border-saffron-100 overflow-hidden group cursor-pointer hover:shadow-2xl transition-all duration-300"
            >
              {/* Pooja Image */}
              ...
              {/* Hover Book Button */}
              <div className="absolute inset-0 flex items-end justify-center pb-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none group-hover:pointer-events-auto">
                <button
                  onClick={(e) => handleBookNow(e, srv.name)}
                  className="bg-[#DA6626] hover:bg-orange-600 text-white font-bold px-8 py-3 rounded-full shadow-xl translate-y-4 group-hover:translate-y-0 transition-transform duration-300"
                >
                  📅 Book Now
                </button>
              </div>
